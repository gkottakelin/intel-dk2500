...
|---|---|
| `depth` | 打印深度帧中心点距离 |
| `depth-viewer` | 深度伪彩可视化窗口，默认开启激光、关闭 LDP/近距离保护 |
| `color-uvc` | 通过 UVC 读取彩色图 |
| `viewer` | 运行 `SimpleViewer.exe`，显示 RGB-D 对齐叠加效果 |
| `pointcloud` | 运行 `GeneratePointCloud.exe`，生成 `.ply` 点云文件 |
| `pointcloud-viewer` | 自动运行并重启点云生成器，连续显示 Viewer 风格点云窗口 |
| `pointcloud-watch` | 不占用相机，只持续观察最新 `.ply` 并刷新点云窗口 |

`viewer` 显示的是彩色图和深度图的二维对齐叠加，不是点云。真正的点云文件由 `pointcloud` 生成。

`GeneratePointCloud.exe` 是官方示例程序，源码中写死了 `MAX_FRAME_COUNT 50`，所以它生成 50 帧点云后会自动停止。项目中的 `pointcloud-viewer` 会在后台自动重启它，解决这个自动停止问题。
...
| 设置 | 默认值 |
|---|---|
| 激光 `OB_PROP_LASER_BOOL` | 开启 |
| LDP/近距离保护 `OB_PROP_LDP_BOOL` | 关闭 |
...
```powershell
python project/src/gemini_windows/run_openni_sample.py depth-viewer --ldp-on
```

如果不想默认开启激光：

```powershell
python project/src/gemini_windows/run_openni_sample.py depth-viewer --laser-off
```
...
1. 后台运行 `GeneratePointCloud.exe`。
2. 自动读取最新生成的 `.ply` 点云。
3. 如果官方生成器到 50 帧停止，自动重启生成器。
...
如果已经有其他程序在生成 `.ply`，只想持续刷新显示：

```powershell
python project/src/gemini_windows/run_openni_sample.py pointcloud-watch
```

...
class CameraVectorTerminalV2Test(unittest.TestCase):
    ...
    def test_forward_below_minus_one_cm_no_longer_triggers_ik_replanning(self):
        """The old combined forward+Z=1cm IK replanning has been removed.
        Z<-1cm forward is now handled by the higher-level counter logic
        in arm_control.py (J3 adjustment + direct grasp trigger)."""
        runtime, controller = self.make_runtime()

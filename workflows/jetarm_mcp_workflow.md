...
6. 控制端首次动作前自动把 J6 设置为松开位置，然后根据抓取点和目标点执行一次 V2 水平对准或下降。
   V2 六方向固定以实际抓取点 XYZ 坐标为准：`forward` 使 Y 减小，`backward` 使 Y 增加，`left` 使 X 减小，`right` 使 X 增加，`up` 使 Z 增加，`down` 使 Z 减小。所有方向保持摄像头—抓取点姿态；请求点因关节限位不可达时，执行本次目标方向上距离请求点最近的可达位置，并在结果中标明限位回退。
   当实际抓取点 `Z<-1cm` 且本步为 `forward` 时，控制端将本步向前距离与`Z=1cm`合成为一个绝对抓取点目标，再重新求解J1-J4；J2不锁定，J3/J4参与整臂协调，并继续保持摄像头—抓取点姿态。
该情况出现次数超过2次时，不再执行合成目标规划，改为：J3位置减小30，运动时间100ms，然后直接实施最终下降与抓取操作。
...
Agent 调用 `run_red_block_grasp` 一次，工具内部全自动执行完整闭环：
采集RGB画面 → HSV红色检测 → V2水平对准/下降 → 重新采集 → 重新检测 → ...
直到完成夹取、J6稳定、回Home。

**AI 参与点严格限制为两个：**
1. **入口**：Agent 判断用户意图，决定调用 `run_red_block_grasp` 启动方式二。
2. **出口**：`run_red_block_grasp` 返回 `autonomous_loop_completed=true` 后，
   Agent 调用 `get_rgb_camera_frame` 获取 Home 后最新画面，
   检查目标物块是否确实被抓起，再调用 `confirm_jetarm_grasp_result(success=...)`。

**AI 不参与中间环节：** 每轮检测-运动决策由程序自动完成，
不经过模型，不消耗 token。

调用前必须先 `initialize_jetarm` 并设置抓取点像素。
若画面中始终未检测到红色物块则返回 `status=error`。

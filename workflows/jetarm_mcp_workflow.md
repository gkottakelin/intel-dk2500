...
3. Agent 只负责在最新 RGB 原图中识别用户指定物品，并把物品中心作为 `target_x/target_y`。坐标原点严格为左上角，X向右、Y向下。
...
4. Agent 调用 `control_jetarm_to_target_pixel(target_x=..., target_y=..., target_vertical_relation=...)`。`target_vertical_relation`只用于交叉校验目标在抓取点上方、下方或同一行，不是机械臂运动方向；仅当中心Y差绝对值不超过2px时使用 `same_y`。
...
7. Agent 必须在新图中重新识别同一物品中心，重新发送 `target_x/target_y`及上下关系，不得复用旧坐标。
...
11. 最终对准后，控制端自动下降到最终抓取高度，先执行夹取并持续读取 J6 位置；只有 J6 在容差内稳定后才返回 Home。若 J6 超时未稳定，则停止抓紧并禁止回 Home。此时结果是 `grasp_completion_status="awaiting_visual_verification"`，不能直接宣称成功。

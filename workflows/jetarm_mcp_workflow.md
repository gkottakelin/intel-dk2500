...
3. Agent 只负责在最新 RGB 原图中识别用户指定物品，并把物品中心作为 `target_x/target_y`。坐标原点严格为左上角 `(0,0)`，X向右、Y向下，右下角为 `(width-1,height-1)`；禁止缩放、翻转或使用比例坐标。
...
4. Agent 调用 `control_jetarm_to_target_pixel(target_x=..., target_y=...)`，只发送目标物品中心像素，不发送或决定机械臂运动方向。

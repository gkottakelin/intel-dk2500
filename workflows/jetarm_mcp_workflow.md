...
8. 如果两个像素点在当前容差内重合，控制程序以 `2 cm/s` 向下运动，并在下降过程中持续基于关节角度/FK 解算抓取点位置。
9. 每下降 `2 cm`，必须重新调用 `get_rgb_camera_frame`，由 Agent 再次寻找目标点像素，然后重新调用 `control_jetarm_to_target_pixel`。
...
13. 如果 Agent 判断抓取失败，必须调用 `set_jetarm_gripper_position(position=370)` 重新松开 J6，再从取图和目标像素查找开始重试。

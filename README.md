...
总控提供Git Pull、机械臂控制、相机显示、基于摄像头的机械臂操控和Agent按钮。
...
机械臂控制、基于摄像头的机械臂操控和Agent可能争用机械臂串口；相机显示和Agent可能
争用Gemini相机。请在打开另一个占用同一硬件的模块前退出当前模块。

“基于摄像头的机械臂操控”启动后会首先弹出抓取点像素配置窗口，默认填入`(320,147)`；
确认后才进入硬件安全确认并连接机械臂。取消或关闭配置窗口会直接退出，不占用机械臂。
